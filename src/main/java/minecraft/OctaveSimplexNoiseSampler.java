...
public class OctaveSimplexNoiseSampler {
    ...
    public OctaveSimplexNoiseSampler(Xoroshiro128PlusPlusRandom random, List<Integer> octaves) {
        this(random, new TreeSet<>(octaves));
    }

    private OctaveSimplexNoiseSampler(Xoroshiro128PlusPlusRandom random, SortedSet<Integer> octaves) {
        int j;
        if (octaves.isEmpty()) {
        ...
        if (j > 0) {
            ...
            Xoroshiro128PlusPlusRandom lv2 = new Xoroshiro128PlusPlusRandom(m);
            for (int n = l - 1; n >= 0; --n) {
